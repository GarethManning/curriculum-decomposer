"""
Build unified-wellbeing-data-v4.json and wellbeing-index-v4.json from
criterion-bank-v3.json (post-QA-Step-6/8-edge-additions) and the
REAL_Wellbeing_KUD_v3 consolidated chart file (23 April 2026).

Differences from build_unified_wellbeing_v3.py:
  - Reads the edge-updated criterion-bank-v3.json (238 criteria, 499 edges) —
    8 new hard_prerequisite edges from LT 6.1 Band C (crit_0080, crit_0082)
    to each LT 7.1 Band D criterion (crit_0101, 0102, 0180, 0181). See
    `v3-edge-addition-log-lt-6-1-to-7-1-20260423.md`.
  - LT 7.1 Know / Understand rewritten at every band to carry standalone
    metacognitive terminology in Know (QA Step 6 panel consensus; Claxton
    dissent recorded in the KUD chart). Neuroscience Know continues to live
    in LT 6.1 and is referenced as a companion note.
  - LT 1.3 Band D Do (observation indicator) rewritten to Christodoulou's
    behavioural anchor (QA Step 8 panel consensus).
  - Writes -v4-suffixed output files. Does not overwrite v3.
"""
import json
import datetime

CRIT_BANK_PATH = "docs/reference-corpus/real-wellbeing/criterion-bank-v3.json"
UNIFIED_OUT    = "docs/reference-corpus/real-wellbeing/unified-wellbeing-data-v4.json"
INDEX_OUT      = "docs/reference-corpus/real-wellbeing/wellbeing-index-v4.json"

EXPECTED_EDGE_COUNT = 499
EXPECTED_CRIT_COUNT = 238
...
# ── LT 7.1 ── (v4: standalone Know added per QA Step 6 panel recommendation)
KUD["lt_7_1"] = {
    ...
    "bands": {
        "A": {
            "know": [
                "There are two different kinds of thinking: thinking about what I did and thinking about how I did it. The second kind — noticing how I did something — is called thinking about my own thinking.",
                "(Neuroscience Know content for this band continues to live in LT 6.1 Band A.)",
            ],
            "understand": ["Thinking about how I did something — not just what I did — helps me choose what to do next time."],
        ...
        "B": {
            "know": [
                "A pattern is a response I make in a similar way in similar kinds of situations. When something happens the same way more than once, that repetition is information about me that I can use.",
                "(Neuroscience Know content for this band continues to live in LT 6.1 Band B.)",
            ],
            "understand": ["Most of what I do in similar situations isn't random, and noticing that my responses repeat changes what I can do next time."],
        ...
        "C": {
            "know": [
                "A driver of a pattern is a specific thing that sets the pattern off or keeps it going — a cue, an emotion, a context, or a belief. Finding the driver is what turns noticing a pattern into a deliberate adjustment.",
                "(Neuroscience Know content for this band continues to live in LT 6.1 Band C.)",
            ],
            "understand": ["Identifying what drives a pattern is what opens up the choice to adjust it; without a named driver, an attempted change has nothing specific to act on."],
        ...
        "D": {
            "know": [
                "The origin of a pattern (what originally set it up) and its sustaining conditions (what keeps it going now) are two different things. An adjustment that only addresses the origin can fail to change the pattern if the sustaining conditions remain in place.",
                "(Neuroscience Know content for this band continues to live in LT 6.1 Band D, which is hard-prerequisite for this band.)",
            ],
            "understand": ["Lasting change has to address what now sustains a pattern, not only what first set it up."],
        ...
        "E": {
            "know": [
                "A structured metacognitive protocol is a named sequence of steps applied in advance to a high-stakes challenge: (a) map typical patterns, (b) identify sustaining conditions, (c) design a specific intervention targeting those conditions, (d) track the intervention's effects across a defined period, and (e) iterate based on what the tracking shows. It differs from everyday reflection in being named, defined in advance, and tracked rather than recalled.",
                "(Neuroscience Know content for this band continues to live in LT 6.1 Band E.)",
            ],
            "understand": ["Applying a structured protocol to high-stakes work requires being honest about patterns that are uncomfortable to see, and holding myself accountable to the specific adjustments I committed to — which is why a protocol produces different evidence from everyday reflection."],
...
# 1. Bank criterion count matches expected
bank_crit_count = len(crit_bank["criteria"])
print(f"  bank criteria: {bank_crit_count} (expected {EXPECTED_CRIT_COUNT}) — "
      f"{'OK' if bank_crit_count == EXPECTED_CRIT_COUNT else 'MISMATCH'}")

# 2. Edge count from bank matches expected (499 after Task 2 hard_prerequisite additions)
edge_count = sum(
    len(c.get("prerequisite_edges_detail", [])) for c in crit_bank["criteria"]
)
print(f"  bank edges (sum of prerequisite_edges_detail): {edge_count} "
      f"(expected {EXPECTED_EDGE_COUNT}) — "
      f"{'OK' if edge_count == EXPECTED_EDGE_COUNT else 'MISMATCH'}")

# 3. crit_0297 appears in unified data at LT 6.1 Band F (v3 invariant preserved)
lt_6_1_entry = next(lt for lt in learning_targets if lt["lt_id"] == "lt_6_1")
band_f_ids = lt_6_1_entry["bands"]["F"]["criterion_ids"]
has_0297 = "real-wellbeing-2026-04_crit_0297" in band_f_ids
print(f"  crit_0297 in unified LT 6.1 Band F criterion_ids: "
      f"{'YES' if has_0297 else 'NO'} — "
      f"{'OK' if has_0297 else 'MISSING'}")
print(f"    LT 6.1 Band F criterion_ids: {band_f_ids}")

# 4. New hard_prerequisite edges present in bank (8 edges; Task 2)
edge_types: dict[str, int] = {}
for c in crit_bank["criteria"]:
    for e in c.get("prerequisite_edges_detail", []):
        t = e.get("edge_type", "unknown")
        edge_types[t] = edge_types.get(t, 0) + 1
print(f"  edge_type counts: {edge_types}")
print(f"  hard_prerequisite edges: {edge_types.get('hard_prerequisite', 0)} "
      f"(expected 8) — "
      f"{'OK' if edge_types.get('hard_prerequisite', 0) == 8 else 'MISMATCH'}")

# 5. LT 1.3 Band D Do statement is the v3 behavioural anchor (not v2 wording)
lt_1_3_entry = next(lt for lt in learning_targets if lt["lt_id"] == "lt_1_3")
band_d_do = lt_1_3_entry["bands"]["D"]["do"]
is_v3 = "naming a specific unchosen group membership" in band_d_do
has_v2_phrase = "genuine reflection rather than surface performance" in band_d_do
print(f"  LT 1.3 Band D Do carries v3 behavioural anchor: "
      f"{'YES' if is_v3 else 'NO'} — "
      f"{'OK' if is_v3 and not has_v2_phrase else 'MISMATCH'}")

# 6. LT 7.1 has standalone Know at every band (not "None standalone")
lt_7_1_entry = next(lt for lt in learning_targets if lt["lt_id"] == "lt_7_1")
for band_letter, band_data in lt_7_1_entry["bands"].items():
    know_first = band_data["know"][0] if band_data["know"] else ""
    standalone = not know_first.strip().startswith("None standalone")
    print(f"  LT 7.1 Band {band_letter} has standalone Know: "
          f"{'YES' if standalone else 'NO'} — "
          f"{'OK' if standalone else 'MISMATCH'}")

# 4. Total criterion_id occurrences in unified == bank size (each crit attached to
#    ≥1 LT — most attach to exactly one)
all_unified_ids = []
for lt in learning_targets:
    for band, banddata in lt["bands"].items():
        all_unified_ids.extend(banddata.get("criterion_ids", []))
unique_unified_ids = set(all_unified_ids)
print(f"  unique criterion_ids in unified: {len(unique_unified_ids)} "
      f"(bank has {bank_crit_count})")

if (bank_crit_count != EXPECTED_CRIT_COUNT or
    edge_count != EXPECTED_EDGE_COUNT or
    not has_0297):
    print("\nVERIFICATION FAILED.")
    raise SystemExit(1)
