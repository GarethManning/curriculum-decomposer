"""
Build unified-wellbeing-data-v3.json and wellbeing-index-v3.json from
criterion-bank-v3.json (post-anchoring-fixes) and the REAL_Wellbeing_KUD_v2
consolidated chart file (23 April 2026).

Differences from build_unified_wellbeing_v2.py:
  - Reads criterion-bank-v3.json in its post-anchoring-fixes state (238 criteria,
    491 edges) — crit_0297 (LT 6.1 Band F action-primacy) now present; 49 new
    cross_lt_source_stated edges from the Band E/F under-anchoring correction.
  - Writes the -v3 suffixed output files.
  - Verifies edge count from bank matches expected (491).
  - Confirms crit_0297 appears in the LT 6.1 Band F criterion_ids of the unified data.

KUD content (Know / Understand / Do / T3 classification / prerequisite_lt_ids)
is unchanged from v2 — this rebuild is driven by bank updates, not chart
updates. The v2 and v3 unified files differ only in the criterion_id lists per
band (crit_0297 addition at LT 6.1 Band F) and the meta block.
"""
import json
import datetime

CRIT_BANK_PATH = "docs/reference-corpus/real-wellbeing/criterion-bank-v3.json"
UNIFIED_OUT    = "docs/reference-corpus/real-wellbeing/unified-wellbeing-data-v3.json"
INDEX_OUT      = "docs/reference-corpus/real-wellbeing/wellbeing-index-v3.json"

EXPECTED_EDGE_COUNT = 491
...
# 2. Edge count from bank matches expected (491 after Task 1 + Task 2)
edge_count = sum(
...
# 3. crit_0297 appears in unified data at LT 6.1 Band F
